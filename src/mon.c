...
void exec_restart_command(monitor_t *monitor, pid_t pid) {
  char buf[1024] = {0};
...
void exec_error_command(monitor_t *monitor, pid_t pid) {
  char buf[1024] = {0};
...
int attempts_exceeded(monitor_t *monitor, int64_t ms) {
  monitor->attempts++;
  monitor->clock -= ms;
...
void start(const char *cmd, monitor_t *monitor) {
exec : {
  pid_t pid = fork();
  int status;
